...
export default function SettingsDialog({ open, locale = 'en', identifyCode, historyRange, error, saving, onIdentifyCodeChange, onHistoryRangeChange, onSubmit, onClose, onLogout }) {
  ...
  return (
    <Dialog open={open} title={zh ? '設定' : 'Settings'} onClose={saving ? undefined : onClose} closeOnBackdrop={!saving} closeLabel={zh ? '關閉設定' : 'Close settings'}>
      <form className="form-stack" onSubmit={onSubmit}>
        <label><span>{zh ? '好友驗證碼' : 'Friend verification code'}</span><input value={identifyCode} onChange={(event) => onIdentifyCodeChange(event.target.value)} placeholder={zh ? '選填' : 'Optional'} /></label>
